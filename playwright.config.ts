...
import { seedTestEnv, REDEYE_CONFIG_PATH } from "./e2e/seed-env";

// Hermetic test environment for e2e specs.
//
// Playwright lifecycle quirk: `webServer.command` boots BEFORE `globalSetup`
// runs, so we cannot rely on globalSetup to populate REDEYE_CONFIG_PATH for
// the dashboard process. We synchronously seed the fixtures here at config
// load time and pass the resolved path through `webServer.env`.
seedTestEnv();
