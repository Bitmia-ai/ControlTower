...
describe("TranscriptViewer — stable keys (T147)", () => {
  it("ToolUseCard preserves open state across SSE replay (index shift)", () => {
    const toolEvent: ClaudeStreamEvent = {
      type: "assistant",
      subtype: "tool_use",
      tool_name: "Bash",
      tool_input: { command: "ls -la" },
    ...
    // Render initial events with the tool_use at index 0.
    const { container, rerender } = render(
      <TranscriptViewer events={[toolEvent]} />
    );
    ...
    // Simulate SSE replay: prepend a __session_boundary__ sentinel and re-include
    // the original tool_use event at index 1. Because the same `_key` is on the
    // same identity event, React should NOT remount the ToolUseCard, so its
    // local open state must persist.
    const boundary = {
      type: "__session_boundary__",
      _key: "__session_boundary__",
    } as unknown as ClaudeStreamEvent;

    rerender(<TranscriptViewer events={[boundary, toolEvent]} />);

    // The "New session" separator must render.
    expect(container.textContent).toContain("New session");

    // The ToolUseCard must still be expanded — its <pre> body is still in DOM
    // because component identity (and thus useState) was preserved by the
    // stable React key.
    pre = container.querySelector("pre");
    expect(pre).toBeTruthy();
    expect(pre!.textContent).toContain("ls -la");

    // The toggle button's aria-expanded must still report open.
    const expandedBtn = screen.getByRole("button", { name: "Collapse tool call" });
    expect(expandedBtn.getAttribute("aria-expanded")).toBe("true");
  });

  it("ThinkingCard preserves open state across index shift when _key is stable", () => {
    const ev: ClaudeStreamEvent = {
      type: "assistant",
      subtype: "thinking",
      content: "Reasoning step content for stable-key test.",
      _key: "msg_thought-0",
    };

    const { container, rerender } = render(<TranscriptViewer events={[ev]} />);

    fireEvent.click(container.querySelector("button")!);
    expect(container.querySelector("p.whitespace-pre-wrap")).toBeTruthy();
    ...
    rerender(<TranscriptViewer events={[boundary, ev]} />);

    // Body still rendered — open state preserved across index shift.
    expect(container.querySelector("p.whitespace-pre-wrap")).toBeTruthy();
  });

  it("falls back to positional key when _key is absent (no regression for fixtures without _key)", () => {
    // Events without _key still render — viewer should not crash when _key is
    // missing. This guards the `event._key ?? \`fallback-${i}\`` defensive
    // branch.
    const ev: ClaudeStreamEvent = {
      type: "assistant",
      subtype: "text",
      content: "no key on this event",
    };
    const { container } = render(<TranscriptViewer events={[ev]} />);
    expect(container.textContent).toContain("no key on this event");
  });
});
