...
export function TranscriptViewer({
  ...
  return (
    <div className="flex flex-col gap-3">
      {events.map((event, i) => {
        // Stable React key — `_key` populated by the normalizer or
        // stream-utils boundary inject. Falls back to a prefixed index for
        // events constructed directly (test fixtures, ad-hoc usage). Using a
        // stable key prevents card remount (and the resulting collapse-state
        // reset + flicker) when the events array is prepended on SSE replay.
        const key = event._key ?? `fallback-${i}`;

        // Session boundary separator — emitted when the tailed file switches
        if ((event.type as string) === "__session_boundary__") {
          return (
            <div key={key} className="flex items-center gap-3 py-2 my-1">
        ...
        if (event.type === "result") {
          return <ResultCard key={key} event={event} />;
        }

        if (event.type === "assistant") {
          if (event.subtype === "tool_use") {
            return <ToolUseCard key={key} event={event} forceOpen={forceExpanded} />;
          }

          if (event.subtype === "thinking") {
            return <ThinkingCard key={key} event={event} forceOpen={forceExpanded} />;
          }

          return <AssistantTextCard key={key} event={event} />;
        }
