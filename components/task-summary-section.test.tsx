...
import { TaskSummarySection } from "./task-summary-section";
...
describe("TaskSummarySection — render gate", () => {
  it("renders nothing when summary is empty string", () => {
    const { container } = render(<TaskSummarySection summary="" />);
  ...
  it("renders the Summary heading when summary is provided", () => {
    render(<TaskSummarySection summary={SHORT} defaultOpen />);
    expect(screen.getByText("Summary")).toBeTruthy();
  });
});

describe("TaskSummarySection — defaultOpen=true (done items)", () => {
  it("shows the full text when opened", () => {
    render(<TaskSummarySection summary={LONG} defaultOpen />);
...
describe("TaskSummarySection — defaultOpen=false (collapsed)", () => {
  it("shows truncated preview with ellipsis when text > 120 chars", () => {
    render(<TaskSummarySection summary={LONG} defaultOpen={false} />);
  ...
  it("shows full text without ellipsis when ≤120 chars", () => {
    render(<TaskSummarySection summary={SHORT} defaultOpen={false} />);
    const preview = screen.getByTestId("summary-preview");
...
describe("TaskSummarySection — toggle interaction", () => {
