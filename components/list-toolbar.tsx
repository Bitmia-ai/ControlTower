...
function SortSelect({
  ...
  // Prefix the rendered option label with "Sort: " so the dropdown is
  // visually distinct from neighbouring filter chips (a sort key like
  // "Priority" used to look identical to a filter named "Priority").
  // The underlying option values stay unprefixed so callers compare by
  // `key` not by display label.
  const activeLabel =
    options.find((o) => o.key === activeSort)?.label ?? activeSort;
  return (
    <div className="relative inline-block">
      <select
        value={activeSort}
        ...
        // Render an explicit "Sort: <label>" string for the closed state
        // by placing it as the (otherwise hidden) selected text. Using a
        // controlled select means the option text is what's shown — so we
        // prefix every option's text. Browsers honour the option text.
      >
        {options.map((opt) => (
