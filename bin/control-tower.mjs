...
// `control-tower init` — delegate to the ct-init wizard.
// Args are passed as an array to spawn (no shell), so there is no
// shell-injection surface. We exit when the child exits — no server start.
if (process.argv[2] === "init") {
  const initScript = join(__dirname, "ct-init.mjs");
  const initChild = spawn(
    process.execPath,
    [initScript, ...process.argv.slice(3)],
    { stdio: "inherit" }
  );
  initChild.on("exit", (code) => process.exit(code ?? 0));
} else {
