 ...
 *
 * Section headers must appear at the start of a line. Earlier versions used
 * `raw.indexOf(header)` which would match a literal "## Discovered" string
 * appearing inside a task's Description prose (e.g. a task about scaffolding
 * tasks.md that names the section literals) — collapsing every section
 * boundary that came after it and silently dropping every task in the real
 * `## Discovered` / `## Triaged` sections from the UI.
 *
 * Match anchored to start-of-line via the multiline `m` flag.
 */
...
function extractSection(raw: string, header: string): string | null {
  ...
  const afterHeader = raw.substring(m.index + m[0].length);
  // Next ## header at start of line; reject ### (sub-items).
  const nextSection = afterHeader.match(/^## (?!#)/m);
...
export function parseSchedules(
  ...
  return headerMatches.map(({ index, id, title }, i) => {
    ...
    // Treat the Unix epoch (or any pre-2001 timestamp) as "never run". The
    // schedules.md template ships SCHED-* placeholders with
    // `Last run: 1970-01-01T00:00:00Z` to mark records that have never executed.
    // Without this guard the UI would render "2938 weeks ago" / "Next: 2938
    // weeks ago" rather than the friendly "Never" / "—" labels and "Never run"
    // status badge.
    let lastRunIso: string | null =
      lastRunRaw && lastRunRaw !== "{ISO timestamp}" && lastRunRaw !== "—"
        ...
        : null;
    if (lastRunIso) {
      const ms = Date.parse(lastRunIso);
      // Year 2001 ≈ 978307200000 ms — anything earlier is a placeholder, not
      // a real schedule run.
      if (!Number.isFinite(ms) || ms < 978307200000) {
        lastRunIso = null;
      }
    }
