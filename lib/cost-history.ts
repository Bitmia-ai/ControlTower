...
export interface SessionHistoryEntry extends SessionCostEntry {
  ...
  phases: string[];
  /**
   * Iteration summaries from `state.json.iteration_log[]` whose `timestamp`
   * falls inside this session's `[startedAt, startedAt+durationMs]` window.
   * Optional — empty/undefined when no log entries match (graceful degradation).
   * See spec T128 / AD-3.
   */
  iterationSummaries?: IterationSummary[];
}

/** Summary of a single iteration that ran inside a session's time window. */
...
/** Default tolerance (ms) for timestamp window matching — handles clock skew. */
const DEFAULT_MATCH_TOLERANCE_MS = 60_000;
/** Widened window (ms) when durationMs is 0 (single-snapshot session). */
const ZERO_DURATION_WINDOW_MS = 5 * 60_000;

/**
 * Pure helper: return iteration_log entries whose `timestamp` falls within
 * the session window `[startedAt, startedAt+durationMs]` (with `tolerance`
 * ms slack on each side). When durationMs is 0, the window widens to ±5 min
 * to handle single-snapshot sessions. Malformed timestamps are skipped.
 *
 * Exported for unit testing — see spec T128 / AD-2.
 */
export function matchIterationLogToSession(
  entries: readonly IterationLogEntry[] | undefined | null,
  startedAt: number,
  durationMs: number,
  tolerance: number = DEFAULT_MATCH_TOLERANCE_MS
): IterationSummary[] {
  if (!entries || entries.length === 0) return [];
  const effectiveDuration = durationMs > 0 ? durationMs : ZERO_DURATION_WINDOW_MS;
  const windowStart = startedAt - (durationMs > 0 ? tolerance : ZERO_DURATION_WINDOW_MS);
  const windowEnd = startedAt + effectiveDuration + tolerance;

  const matched: IterationSummary[] = [];
...
export async function getSessionHistory(
  ...
  // Read state.json once; tolerate missing/malformed file (graceful degradation).
  let iterationLog: IterationLogEntry[] | undefined;
  try {
    const state = await readState(projectPath);
    iterationLog = state?.iteration_log;
  } catch {
  ...
  return mapWithConcurrency(recent, HISTORY_FANOUT_CONCURRENCY, async (c) => {
    ...
    const iterationSummaries = matchIterationLogToSession(
      iterationLog,
      startedAt,
      durationMs
    );
