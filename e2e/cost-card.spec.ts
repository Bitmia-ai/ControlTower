...
test.describe("Cost card (T049)", () => {
  test("renders cost card with non-negative values and upholds session <= total invariant", async ({
    ...
    //    cost values render. The current CostCard uses "Session" / "Total"
    //    labels (T067 stacked rail layout, not the original "this session
    //    (est.)" / "total (est.)" labels).
    const sessionLabel = page.getByText("Session", { exact: true });
    await expect(sessionLabel).toBeVisible({ timeout: 5000 });

    // 4. Extract session cost value
    const sessionRow = sessionLabel.locator("xpath=..");
    const sessionCostText = await sessionRow
    ...
    const totalCostText = await totalRow
