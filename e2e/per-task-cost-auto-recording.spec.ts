...
test.describe("T046 — auto cost recording", () => {
  ...
  test("cost-snapshot endpoint responds for a valid blId", async ({ request }) => {
    // After the cost-start test above, T046-e2e-test has a baseline recorded.
    ...
    const res = await request.post("http://localhost:3200/api/projects/0/cost-snapshot", {
      data: { blId: "T046-e2e-test" },
    });
    expect(res.status()).toBe(200);
    const json = await res.json();
    expect(json.data.blId).toBe("T046-e2e-test");
