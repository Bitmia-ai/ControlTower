...
test.describe("Steer tab (T063)", () => {
  test("renders Steer tab, submits a directive, refreshes the list", async ({
    ...
    // 3. Verify the page mounted (T155 retired the redundant inner h1
    // "Steer"; the compose card was promoted instead, so we assert the
    // form's textarea rendered. Use the role-based locator to avoid the
    // strict-mode collision with the section's aria-label "Send a new
    // directive" (which substring-matches "New directive").
    await expect(
      page.getByRole("textbox", { name: "New directive" })
    ).toBeVisible();
    ...
    // 6. Type a directive
    const textarea = page.getByRole("textbox", { name: "New directive" });
