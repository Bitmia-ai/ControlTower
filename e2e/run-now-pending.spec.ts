...
test.describe("T125 — Run Now creates a pending task entry", () => {
  // The Run Now endpoint auto-starts the CTO session via startSession(),
  // which spawns the `claude` CLI binary. CI runners don't have that binary
  // installed, so the endpoint returns 500 and the queued state never
  // appears. Skip in CI; the route's behaviour is covered by unit tests
  // for the endpoint itself (app/api/.../schedules/run/route.test.ts).
  test.skip(
    !!process.env.CI,
    "Requires the `claude` CLI binary, which is not installed on CI runners"
  );
