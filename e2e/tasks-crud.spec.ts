...
test.describe("Backlog CRUD (T049)", () => {
  test("add a task via dialog, verify it appears in list, navigate to detail", async ({
    ...
    // 2. Click the "Add Item" button to open dialog (header has one;
    //    the empty-state body has another — use the first match).
    const addBtn = page
    ...
    // 10. Verify the title is displayed on the detail page (h1 heading;
    //    the page header also has an h2 with the same text)
    await expect(
      page.getByRole("heading", { name: "My new test task", level: 1 })
