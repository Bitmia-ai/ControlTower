...
test.describe("Phase-change toast notifications (T050)", () => {
  test("toast appears when phase transitions PLAN -> BUILD via polling", async ({
    ...
    // Time-based phase switch — counting individual GETs is unreliable
    // because layout, mission-control, and notification-bell components all
    // hit /api/projects/0 at mount. A monotonic clock guarantees the page
    // observes PLAN as baseline before BUILD arrives on the next 5s poll.
    const startedAt = Date.now();
    const PHASE_SWITCH_MS = 4_000;

    await page.route("**/api/projects/0", async (route) => {
      if (route.request().method() === "GET") {
        const phase = Date.now() - startedAt < PHASE_SWITCH_MS ? "PLAN" : "BUILD";
        await route.fulfill({
          status: 200,
