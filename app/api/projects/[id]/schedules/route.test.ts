...
// Stub commit so the POST handler doesn't shell out during tests.
...
describe("POST /api/projects/[id]/schedules", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockCommitAndPush.mockResolvedValue({ committed: true });
...
describe("parseProjectIndex bad-id guard", () => {
  ...
  it("POST returns 400 for non-numeric id", async () => {
    const [req, ctx] = makePost("abc", { name: "x", frequency: "daily", steps: ["s"] });
    const res = await POST(req, ctx);
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe("id must be a non-negative integer");
