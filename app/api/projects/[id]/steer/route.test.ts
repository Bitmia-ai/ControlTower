...
describe("PATCH /api/projects/[id]/steer", () => {
  ...
  it("returns 200 and writes the edited file on happy path", async () => {
    ...
    const json = await res.json();
    expect(json.data.success).toBe(true);
    // atomicWriteJson was called with the file path + the patched content.
...
describe("parseProjectIndex bad-id guard", () => {
  ...
  it("POST returns 400 for non-numeric id", async () => {
    const [req, ctx] = makePostRequest("abc", { directive: "x" });
    const res = await POST(req, ctx);
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe("id must be a non-negative integer");
  ...
  it("PATCH returns 400 for non-numeric id", async () => {
    const [req, ctx] = makeMutateRequest("PATCH", "abc", { index: 0, text: "x" });
    const res = await PATCH(req, ctx);
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe("id must be a non-negative integer");
  ...
  it("DELETE returns 400 for non-numeric id", async () => {
    const [req, ctx] = makeMutateRequest("DELETE", "abc", { index: 0 });
    const res = await DELETE(req, ctx);
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toBe("id must be a non-negative integer");
