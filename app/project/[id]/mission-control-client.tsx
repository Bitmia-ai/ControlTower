...
import { VelocityCard } from "@/components/mission-control/velocity-card";
...
export default function MissionControlClient({
  ...
  // The body MUST use the field name `taskId` — both `/cost-start` and
  // `/cost-snapshot` route handlers validate against TASK_ID_RE on `taskId`.
  // Earlier code mis-typed the field as `blId` and the routes responded 400.
  const activeId = detail ? (detail.state?.task_id ?? null) : undefined;
  const postCostStart = useCallback((taskId: string) => {
    // Fire-and-forget — failure is non-fatal; server-side idempotency covers retries.
  ...
  const postCostSnapshot = useCallback((taskId: string) => {
    // Fire-and-forget — see postCostStart.
  ...
  async function handleRestart() {
    try {
      ...
      // ignore — restart errors are surfaced via the next fetchDetail() poll.
    }
  }
  ...
  return (
    <main className="px-4 sm:px-6 pb-12 max-w-6xl mx-auto overflow-x-hidden">
      {loading && !detail ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-5 lg:items-start min-w-0">
          {/*
          ...
          */}
          ...
          <div className="flex flex-col gap-4">
            ...
            <VelocityCard projectId={projectId} running={running} />
