...
describe("BacklogSection — author badge + filter (T134/T135)", () => {
  // The filter dropdown also contains the labels "Created by User" / "Created
  // by RedEye" as <option> text, so to test row-level rendering we look for the
  // badge spans (recognisable by the blue/violet Tailwind classes) rather than
  // raw textContent.
  function authorBadgeLabels(container: HTMLElement): string[] {
    ...
    return spans
      .filter(
        (s) =>
          s.className.includes("bg-blue-50") ||
          s.className.includes("bg-violet-50")
      )
      .map((s) => s.textContent ?? "");
  }

  it("renders 'Created by User' badge for ceo-section items", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T200", status: "pending", section: "ceo", title: "User-submitted" }),
    ];
    const { container } = render(
      <BacklogSection items={items} projectId={0} />
    );
    expect(authorBadgeLabels(container)).toEqual(["Created by User"]);
  });

  it("renders 'Created by RedEye' badge for discovered-section items", () => {
    const items: TaskItem[] = [
      makeItem({
        id: "T201",
        status: "pending",
    ...
    expect(authorBadgeLabels(container)).toEqual(["Created by RedEye"]);
  });

  it("renders both badges in a mixed-author backlog", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T210", status: "pending", section: "ceo", title: "From CEO" }),
      makeItem({
        id: "T211",
        status: "pending",
        section: "discovered",
    ...
    expect(authorBadgeLabels(container).sort()).toEqual([
      "Created by RedEye",
      "Created by User",
    ]);
  });

  it("exposes a 'Created by' filter chip in the toolbar", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T220", status: "pending", section: "ceo" }),
    ];
    render(<BacklogSection items={items} projectId={0} />);
    // FilterChip renders an aria-labelled <select>; the Created by chip
    // should appear alongside Priority and Status.
    const authorSelect = screen.getByLabelText("Filter by Created by");
    expect(authorSelect).toBeTruthy();
    expect((authorSelect as HTMLSelectElement).tagName).toBe("SELECT");
  });

  it("filters to only ceo items when author=user is selected", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T230", status: "pending", section: "ceo", title: "From CEO" }),
      makeItem({
        id: "T231",
        status: "pending",
        section: "discovered",
    ...
    const authorSelect = screen.getByLabelText("Filter by Created by") as HTMLSelectElement;
    fireEvent.change(authorSelect, { target: { value: "user" } });
    expect(container.textContent).toContain("From CEO");
    expect(container.textContent).not.toContain("From bot");
  });

  it("filters to only RedEye items when author=redeye is selected", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T240", status: "pending", section: "ceo", title: "From CEO" }),
      makeItem({
        id: "T241",
        status: "pending",
        section: "discovered",
    ...
    const authorSelect = screen.getByLabelText("Filter by Created by") as HTMLSelectElement;
    fireEvent.change(authorSelect, { target: { value: "redeye" } });
    expect(container.textContent).toContain("From bot");
    expect(container.textContent).not.toContain("From CEO");
  });

  it("clearing filters via 'Clear' resets the author filter", () => {
    const items: TaskItem[] = [
      makeItem({ id: "T250", status: "pending", section: "ceo", title: "From CEO" }),
      makeItem({
        id: "T251",
        status: "pending",
        section: "discovered",
