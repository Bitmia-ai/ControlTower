...
describe("SteerContent", () => {
  ...
  it("renders compose form and Active Directives heading immediately", () => {
    vi.spyOn(global, "fetch").mockReturnValue(new Promise(() => {}));
    render(<SteerContent id="0" />);
    // The redundant inner h1 "Steer" was removed — assert the new compose card
    // label and Active Directives section header instead.
    expect(screen.getByLabelText("New directive")).toBeDefined();
    expect(screen.getByRole("button", { name: /Send Directive/i })).toBeDefined();
    expect(
      screen.getByRole("heading", { name: "Active Directives", level: 2 })
    ).toBeDefined();
  });

  it("does not render the redundant inner Steer h1 (T155)", () => {
    vi.spyOn(global, "fetch").mockReturnValue(new Promise(() => {}));
    render(<SteerContent id="0" />);
    // The compose card was promoted; the inner page-level "Steer" h1 should not exist.
    expect(
      screen.queryByRole("heading", { name: "Steer", level: 1 })
    ).toBeNull();
  });

  it("character counter is hidden when textarea is empty and visible after typing", async () => {
    vi.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ data: { directives: [] } }),
    } as Response);
    render(<SteerContent id="0" />);
    // Empty: counter not visible
  ...
  it("submit disabled and counter red when over MAX_DIRECTIVE_CHARS limit", async () => {
    vi.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ data: { directives: [] } }),
    } as Response);
    ...
    const button = screen.getByRole("button", {
      name: /Send Directive/i,
    }) as HTMLButtonElement;
    expect(button.disabled).toBe(true);
  });

  it("renders count badge in section header when directives exist", async () => {
    const directives = [
      makeDirective("First (2026-04-25)"),
      makeDirective("Second (2026-04-25)"),
      makeDirective("Third (2026-04-25)"),
    ];
    vi.spyOn(global, "fetch").mockResolvedValue({
      ok: true,
      json: async () => ({ data: { directives } }),
    ...
    // Count badge: a span containing exactly "3" near the Active Directives heading.
    const heading = screen.getByRole("heading", {
      name: "Active Directives",
      level: 2,
    });
    const sectionHeader = heading.parentElement!;
    const badge = Array.from(sectionHeader.querySelectorAll("span")).find(
      (s) => s.textContent === "3"
    );
